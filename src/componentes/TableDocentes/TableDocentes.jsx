		...
		const notasCursoDocente = async () => {
			try {
				...
				if (data.length > 0) {
					...
					setNombres(
						data
							.map((item) => ` ${item.apellido} ${item.nombre}`)
							.sort((a, b) =>
			...
			<div className='grupo'>
				...
				<div
					className={`contenedor ${
						primerClick ? (expandir ? 'expandir' : 'noExpandir') : 'noMostrar'
					}`}
				>
					<PildoraTitulo
						nota={infoNota}
					...
					>
						<div className='titulo'>
							...
							<p>
								Total de porcentaje ocupado: {totalPorcentajes}%,
								disponible para asignar: {100 - totalPorcentajes}%
							</p>
							{totalPorcentajes === 100 && (
								<p className='alertaTxt'>
									Edita el peso de actividades para poder asignar
									más.
								</p>
							)}
						</div>
						<div className='crearAct'>
							<div className='campos'>
								...
								<button
									type='submit'
									disabled={cargando || totalPorcentajes === 100}
								>
									{cargando
										? 'cargando...'
										: totalPorcentajes === 100
